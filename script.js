...
class Letter {
  ...
  _stepFirework() {
    ...
    for (let i = 1; i < this.prevPoints.length; i++) {
      ...
      const alpha = (i / this.prevPoints.length) * 0.9;
    ...
    ctx.beginPath();
    ctx.arc(head[0], head[1], head[2] * 0.5, 0, Tau);
    ctx.fillStyle = this.col.alpha(0.9);
    ctx.fill();
    ...
    if (this.tick >= this.reachTime) {
      ...
      this.circleCompleteT =
        (opts.fireworkCircleBaseTime +
          opts.fireworkCircleAddedTime * Math.random()) |
        0;
      this.circleFadeT =
        (opts.fireworkCircleFadeBase +
          opts.fireworkCircleFadeAdded * Math.random()) |
        0;
      ...
      // spawn shards
      const count =
        (opts.fireworkBaseShards + opts.fireworkAddedShards * Math.random()) |
        0;
  ...
  _stepBalloon() {
    ...
    if (this.spawning) {
      ...
      ctx.save();
      ctx.shadowColor = this.col.alpha(0.5);
      ctx.shadowBlur = 18;
      ...
      const grad = ctx.createRadialGradient(
        this.cx - this.size * 0.25,
        this.cy - this.size * 0.6,
        this.size * 0.05,
        this.cx,
        this.cy - this.size * 0.4,
        this.size * 0.7,
      );
      ...
      // string
      ctx.lineWidth = 1;
      ctx.strokeStyle = this.col.light(80, 0.6);
      ctx.beginPath();
      ctx.moveTo(this.cx, this.cy);
      ...
      ctx.font = `bold ${opts.charSize * 0.75}px 'Playfair Display', serif`;
      ctx.fillText(
        this.char,
        this.cx + this.dx * 0.75,
        this.cy + this.dy * 0.4,
      );
      ctx.font = `bold ${opts.charSize}px 'Playfair Display', serif`;
...
function anim() {
  ...
  // vignette overlay
  const vig = ctx.createRadialGradient(hw, hh, hh * 0.4, hw, hh, hw * 1.3);
...
document
  ...
  .addEventListener("click", burstConfetti);
